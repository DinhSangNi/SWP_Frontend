...
const CourseDetail = () => {
    ...
    useEffect(() => {
        const fetchAnnouncements = async () => {
            try {
                const response = await announcemmentsApi.getAnnouncemmentsByIdCourse(id);
                const allAnnouncements = response.data.$values;
                setAnnouncements(allAnnouncements);
                
                // Luôn hiển thị 5 thông báo đầu tiên
                setVisibleAnnouncements(allAnnouncements.slice(0, announcementsPerPage));
    ...
    return (
        ...
        >
            <div className="flex justify-between">
                            ...
                            {showAnnouncements && (
                                ...
                                >
                                    <div className="flex items-center p-3 bg-gradient-to-r from-purple-600 to-blue-500">
                                        ...
                                        <h3 className="text-lg font-bold text-white">Thông báo khóa học</h3>
                                    </div>
                                    <div 
                                        id="scrollableDiv"
                                    ...
                                    >
                                        <InfiniteScroll
                                            dataLength={visibleAnnouncements.length}
                                            ...
                                            loader={
                                                ...
                                                >
                                                    ...
                                                    <p className="mt-2 text-purple-600">Đang tải thêm...</p>
                                                </motion.div>
                                            }
                ...
                <div className="text-black basis-3/5">
                    <div className="w-full py-4 px-28">
                        {loading ? (
                            <Skeleton className="w-1/3 h-full my-5" loading={loading} active paragraph={{ rows: 6 }} />
                        ) : (
                            <>
                                <h1 className="my-3 text-3xl font-bold">{course?.courseName}</h1>
                                <p className="my-3 text-xl">{course?.description}</p>
                                <p className="text-[1rem]">
                                    Created by <span className="font-bold text-purple-600 hover:underline">
                                        <Link to="">{course?.createdBy.fullName}</Link>
                                    </span>
                                </p>
                                <p className="text-[0.8rem] flex items-center gap-1">
                                    <MdDateRange className="text-[1rem] text-black" />
                                    From <span>{course?.startDate}</span> To <span>{course?.endDate}</span>
                                </p>
                                <div className="flex items-center gap-3">
                                    ...
                                    <p className="text-[0.8rem] flex items-center gap-1">
                                        <MdLanguage className="text-black text-[1rem]" /> VietNamese
                                    </p>
                                </div>
                            </>
                        )}
            ...
            >
                <CourseCarousel heading={<h1 className="text-2xl font-bold">You also might like</h1>} items={courses} loading={carouselLoading} />
            </motion.div>
        </motion.div>
    );
};
